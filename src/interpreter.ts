...
import { type Gas, type IGasCounter, Status, tryAsGas } from "./pvm-types.js";
...
export class Interpreter {
	...
	private setMemorySegment(
		seg: { start: number; end: number; data: Uint8Array | null },
		access: PageAccess,
	): void {
		if (seg.data !== null && seg.data.length > 0) {
			...
			while (addr < seg.end && offset < seg.data.length) {
				...
				if (page === undefined) {
					...
					if (seg.data !== null) {
						page.data.set(
							seg.data.subarray(offset, offset + toCopy),
							pageOffset,
						);
					}
				}
	...
	nextStep(): Status {
		...
		const result = this.dispatch[opcode](
			this.ctx,
			this.pc,
			this.code,
			this.skip,
